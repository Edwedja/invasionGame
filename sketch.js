...
function draw() {
  ...
  //colocando o background para tocar! 
  if (!backgroundMusic.isPlaying()) { // para não ficar em loop!! 
...
function showCannonBalls(ball, index) {
  if (ball) {
    ...
    if (ball.body.position.x >= width || ball.body.position.y >= height - 50) {
      if (!ball.isSink){
        waterSound.play() // add SOM da água
        ball.remove(index);
      } 
    ...
    for (var i = 0; i < boats.length; i++) {
      if (boats[i]) {
        ...
        if (collision.collided  && ! boats[i].isBroken){
          // verificar se o pirata ja está rindo: 
...
function keyReleased() {
  if (keyCode === DOWN_ARROW) {
    cannonExplosion.play(); // som de explos;ao 
